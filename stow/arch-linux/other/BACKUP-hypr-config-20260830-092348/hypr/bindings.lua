...
-- Ar's close-window binding. SUPER + Q is unbound in the current defaults.
...
-- Use Ctrl+B to collapse or expand browser side panels while preserving the
-- normal shortcut in every other application.
local function send_ar_shortcut_once(mods, key)
...
o.bind("CTRL + B", "Toggle browser side panel", function()
  local window = hl.get_active_window()
  if window and (window.class == "chromium" or window.class == "helium") then
    send_ar_shortcut_once("CTRL SHIFT", "L")
  elseif window and window.class == "zen" then
    send_ar_shortcut_once("CTRL ALT", "Z")
  else
    send_ar_shortcut_once("CTRL", "B")
  end
end)
